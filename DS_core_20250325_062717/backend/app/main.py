...
from fastapi import FastAPI, HTTPException, Depends, UploadFile, File, status, Query, BackgroundTasks, Request, Response, Form, Body, Security
...
from .schemas.paper import Paper as PaperSchema, PaperCreate, PaperUpdate, PaperWithTags
...
# 导入路由模块
from .routers import papers, users, notes, knowledge_graph, recommendations, projects, publication_rank, search
...
# 添加直接的游客登录路由
@app.post("/api/users/guest", response_model=Token)
async def login_as_guest_directly(db: Session = Depends(get_db)):
    """直接提供游客登录功能的路由，路径为 /api/users/guest"""
    try:
        # 调用用户路由中的游客登录逻辑
        from .dependencies import create_guest_user
...
# 添加用户论文路由
@app.get("/api/users/me/papers", response_model=List[PaperSchema])
async def get_current_user_papers(
    skip: int = 0,
    limit: int = 100,
    db: Session = Depends(get_db),
    ...
    """获取当前用户的所有论文"""
    try:
        logger.info(f"主路由: 开始获取用户论文，用户ID: {current_user.id}, 跳过: {skip}, 限制: {limit}")
        papers = db.query(Paper).filter(Paper.user_id == current_user.id).offset(skip).limit(limit).all()
        logger.info(f"主路由: 成功检索到论文数量: {len(papers)}")
        return papers
    except Exception as e:
        logger.error(f"主路由: 获取用户论文失败: {str(e)}")
        logger.error(f"主路由: 错误详情: {traceback.format_exc()}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"获取用户论文失败: {str(e)}"
        )
