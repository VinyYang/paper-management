    ...
    try:
        ...
        for key, group in link_groups.items():
            ...
            links.append({
                "source": group["source"],
                "target": group["target"],
                "value": " / ".join(relation_labels),
                "weight": group["total_weight"] / len(group["relations"]),
                "relationCount": len(group["relations"]),
                # 为了视觉效果，设置不同的曲率以避免重叠
                "curvature": 0.1 + (0.1 * (len(group["relations"]) - 1)) if len(group["relations"]) > 1 else 0.1
            })
        ...
        return {
            ...
            "links": links,
            "categories": [
                {"name": "基础概念", "itemStyle": {"color": "#1890ff"}},
                {"name": "领域概念", "itemStyle": {"color": "#52c41a"}},
                {"name": "主题概念", "itemStyle": {"color": "#722ed1"}},
                {"name": "方法概念", "itemStyle": {"color": "#fa8c16"}},
                {"name": "工具概念", "itemStyle": {"color": "#eb2f96"}}
            ],
            "stats": {
...
async def calculate_paper_similarity(
    ...
    for p in user_papers_query:
        ...
        # 计算综合相似度
        comprehensive_similarity = (
            default_weights["concept"] * concept_similarity +
            default_weights["title"] * title_similarity +
            default_weights["author"] * author_similarity +
            default_weights["venue"] * venue_similarity +
            default_weights["year"] * year_similarity
        )
